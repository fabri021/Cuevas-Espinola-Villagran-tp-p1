		...
		for (int i = 0; i < zomI; i++) {
			...
			if (zombie[i] != null) {
				...
				for (int p = 0; p < filas; p++) {
					...
					if(roseBlade != null && colisionConPlanta(zombie[i], roseBlade)&& roseBlade.colocada) {
				...
				boolean colisionTanque = false;

				for (int t = 0; t < filas; t++) {
					Tanque tanque = Tung[t];
					if(tanque != null && colisionConTanque(zombie[i], tanque)&& tanque.colocada) {
				        colisionTanque = true;
